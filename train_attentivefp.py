...
train_data_path = './CSV/HIV_train.csv'
val_data_path = './CSV/HIV_val.csv'
test_data_path = './CSV/HIV_test.csv'
...
class AttentiveFPModel(torch.nn.Module):
    def __init__(self, input_dim=1, hidden_dim=hidden_dim, edge_dim=1, num_layers=1, num_timesteps=1, output_dim=1):
        ...
        self.layer2 = AttentiveFP(hidden_dim, hidden_dim, output_dim, edge_dim, num_layers, num_timesteps)
...
def mol_to_pyg_data(mol, bonds, bond_types):
    ...
    #     mass += atom.GetMass()# + atom.GetNumImplicitHs()
...
def create_dataloader(df, shuffle=True):
    ...
    for i in trange(len(df), desc='Creating data loader'):
        ...
        label = df.loc[i, 'HIV_active'].astype(float)
    ...
    # with open('./PKL/HIV_test_w_mass.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    train_loader = create_dataloader(df_train, shuffle=False)
    val_loader = create_dataloader(df_val, shuffle=False)
    test_loader = create_dataloader(df_test, shuffle=False)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for data in tqdm(val_loader):
                ...
                predicted_score = model.layer2(node_attr.squeeze(), edge_index, edge_attr, batch)
                logit = torch.sigmoid(predicted_score)
                LABEL.append(label[0].item())
                LOGIT.append(logit[0][0].item())
    ...
    with torch.no_grad():
        for data in tqdm(test_loader):
            ...
            # print(f'Groundtruth: {round(float(label.squeeze()), 3)}, Predicted: {round(float(logit.squeeze()), 3)}')
            # test_loss += loss.item()
            LABEL.append(label[0])
            LOGIT.append(logit[0][0])
